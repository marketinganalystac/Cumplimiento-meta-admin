...
interface ConsoleViewProps {
  ...
  maestroCSVUpdatedAt?: string | null;
  onMaestroCSVLoad: (text: string, name: string) => void;
  maestroCsvLoading?: boolean;
  isAdmin: boolean;
  userEmail: string;
  onLogout: () => void;
}
...
function WrenchIcon() {
  return (
    <FooterIconWrap>
      <path strokeWidth="1.7" transform="translate(-3.8,-3.8) scale(1.2)" d="M14.7 6.3a4 4 0 0 0-5.4 5.4L3 18l3 3 6.3-6.3a4 4 0 0 0 5.4-5.4l-2.8 2.8-2-2Z" />
    </FooterIconWrap>
  );
}
...
export default function ConsoleView({
  ...
  maestroCSVName, maestroCSVUpdatedAt, onMaestroCSVLoad, maestroCsvLoading,
  isAdmin, userEmail, onLogout,
}: ConsoleViewProps) {
  const csvInputRef = useRef<HTMLInputElement>(null);
  const tallerCsvInputRef = useRef<HTMLInputElement>(null);
  const maestroCsvInputRef = useRef<HTMLInputElement>(null);
  ...
  const [maestroLoaded, setMaestroLoaded] = useState(false);
  const [maestroLabel, setMaestroLabel] = useState('Maestro Clientes');
  const [maestroProgress, setMaestroProgress] = useState<number | null>(null);
  const [maestroLoadedAt, setMaestroLoadedAt] = useState<string | null>(null);
  ...
  useEffect(() => {
    if (maestroCSVName && !maestroCsvLoading) {
      setMaestroLoaded(true);
      setMaestroLabel(maestroCSVName);
      const formatted = formatTimestamp(maestroCSVUpdatedAt);
      if (formatted) setMaestroLoadedAt(formatted);
    }
  }, [maestroCSVName, maestroCSVUpdatedAt, maestroCsvLoading]);
  ...
  function handleCSV(input: HTMLInputElement, kind: 'general' | 'taller' | 'maestro') {
    const file = input.files?.[0];
    if (!file) return;
    const setProgress = kind === 'taller' ? setTallerProgress : kind === 'maestro' ? setMaestroProgress : setCsvProgress;
    setProgress(0);
    ...
    reader.onload = function(e) {
      ...
      if (kind === 'taller') {
        ...
        onMaestroCSVLoad(text, file.name);
        setMaestroLoaded(true);
        setMaestroLabel(file.name);
        setMaestroLoadedAt(horaActual());
      } else {
    ...
    <div className="ac-wrap">
          ...
          {isAdmin && (
            <>
              ...
              <input type="file" ref={csvInputRef} className="csv-chip-input" accept=".csv,.txt"
                onChange={(e) => handleCSV(e.target as HTMLInputElement, 'general')} />
              ...
              <input type="file" ref={tallerCsvInputRef} className="csv-chip-input" accept=".csv,.txt"
                onChange={(e) => handleCSV(e.target as HTMLInputElement, 'taller')} />

              <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
                <button className="csv-chip" onClick={() => maestroCsvInputRef.current?.click()}
                  title="Cargar Maestro Clientes — Créditos" disabled={maestroCsvLoading}
                  style={{ borderColor: maestroLoaded ? 'rgba(39,174,96,.5)' : undefined }}>
                  <span className={`csv-dot${maestroLoaded ? ' ok' : ''}`}></span>
                  <i className="fas fa-upload"></i>
                  <span>{maestroCsvLoading ? 'Cargando...' : maestroLabel}</span>
                </button>
                {maestroProgress !== null && (
                  <div style={{ width: '100%', height: '4px', background: 'rgba(255,255,255,.12)', borderRadius: '3px', overflow: 'hidden' }}>
                    <div style={{
                      height: '100%', borderRadius: '3px',
                      width: `${Math.min(100, maestroProgress)}%`,
                      background: 'linear-gradient(90deg,#F5C518,#27ae60)',
                      transition: 'width .15s ease-out',
                    }} />
                  </div>
              ...
              <input type="file" ref={maestroCsvInputRef} className="csv-chip-input" accept=".csv,.txt"
                onChange={(e) => handleCSV(e.target as HTMLInputElement, 'maestro')} />
            </>
          )}
      ...
      <div className="ac-grid">
        ...
        <div className="ac-card">
          <div className="ac-card-head">
            <div className="ac-icon">
              <svg viewBox="0 0 24 24" fill="none" stroke="#F5C518" strokeWidth="1.7">
